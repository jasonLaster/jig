...
The two default lengthwise rails are 2.5 in high × 2.5 in wide and add mirrored 2.5 in true-circular upper-end returns. In side elevation, each return stays tangent to the tabletop-facing top and descends through the full rail height at the leg frame, echoing the aggressive circular shoulder of the transverse Wave frames. The tabletop-facing top round-over and the return's end face-edge round-over are independent controls. The top can grow to the full safe radius derived from the rail width and height; changing it never changes the end treatment. The end face-edge uses the same 3/8 in round-over as the legs by default, with a URL-backed “Match the end face-edge to the legs” control. The full-height return is intentionally fabrication-intensive and requires engineered positive joinery rather than relying on end bearing.
